...
export default function App() {
  ...
  return (
    <SafeAreaView style={styles.screen}>
      <Text style={styles.badge}>Chapter 08</Text>
      <Text style={styles.title}>프로필 상세 보기</Text>
      <Text style={styles.subtitle}>
        목록에서 선택한 프로필의 상세 정보를 아래 카드에서 확인합니다.
      </Text>
      ...
      <View style={styles.sortRow}>
        <Pressable
          style={[styles.sortButton, sortKey === 'follow' && styles.sortButtonActive]}
          onPress={() => setSortKey('follow')}
        >
          <Text
            style={[styles.sortButtonText, sortKey === 'follow' && styles.sortButtonTextActive]}
          >
            팔로잉 우선
          </Text>
        </Pressable>
        ...
        >
          <Text
            style={[styles.sortButtonText, sortKey === 'name' && styles.sortButtonTextActive]}
          >
            이름순
          </Text>
        </Pressable>
      ...
      {selectedProfile ? (
        <View style={styles.detailCard}>
          <Text style={styles.detailLabel}>선택한 프로필</Text>
          <Text style={styles.detailName}>{selectedProfile.name}</Text>
          <Text style={styles.detailRole}>{selectedProfile.role}</Text>
          <Text style={styles.detailText}>
            {selectedProfile.following
              ? '현재 팔로잉 중이며, 새 글 알림을 받고 있습니다.'
              : '아직 팔로우하지 않았습니다. 목록 버튼으로 상태를 바꿔 보세요.'}
          </Text>
        </View>
      ) : null}
